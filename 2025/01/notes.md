...
Total time is just a little over 1ms. A nice thing about my solution is that it's O(n) where n is number of instructions, rather than O(n) where n is number of ticks. This trades off time complexity for code complexity.
