...
class Grid:
    ...
    def storms_at_t(self, t: int, cache: dict = {}) -> set[Storm]:
        """Return storms along with their direction at time t. Only needed for show_at_t."""
        if t in cache:
            return cache[t]
        ...
        for storm in self.storms:
            ...
            nx = (sx + t * dx) % self.width
            ny = (sy + t * dy) % self.height
            result.add(((nx, ny), (dx, dy)))

        cache[t] = result

        return result

    def show_at_t(self, t: int, point: Point | None = None):
        """Just for debugging."""
        grid = [["."] * self.width for _ in range(self.height)]

        storms = self.storms_at_t(t)
        for (sx, sy), dir_ in storms:
            grid_char = str(grid[sy][sx])
            if grid_char == ".":
                grid[sy][sx] = DIR_UNMAP[dir_]
            elif grid_char in "v^<>":
                grid[sy][sx] = "2"
            else:
                grid[sy][sx] = str(int(grid_char) + 1)

        if point is not None:
            grid[point[1]][point[0]] = "E"
